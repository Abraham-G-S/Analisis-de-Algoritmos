def fuerzaBruta():
    print("Algoritmo de optimización de Cartera (Sharpe simplificado)\n")

    activos = 0
    pasos = 0
    rendimiento = []  #rendimiento esperado por activo
    variacion = []    # desviacioneos estandar por activo
    ...
    # Evaluar una combinación de pesos k_vec en "unidades"
    def evaluar(k_vec):
        nonlocal mejor_sharpe, mejor_pesos, mejor_r, mejor_sigma
        pesos = [k / unidades for k in k_vec]  # pesos en 0,1 que suman 1

        # Retorno esperado de cartera: suma ponderada de retornos
        r_port = sum(p * r for p, r in zip(pesos, rendimiento))

        # Volatilidad de cartera: suma ponderada de volatilidades individuales
        # Nota: Es una aproximación pedagógica. La fórmula correcta usa covarianzas.
...
#combinaciones usando fuerza bruta
    if activos == 1:
